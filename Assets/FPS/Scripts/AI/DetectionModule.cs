...
namespace Unity.FPS.AI
{
    public class DetectionModule : MonoBehaviour
    {
        ...
        public virtual void HandleTargetDetection(Actor actor, Collider[] selfColliders, float visionRange, float visionAngle)
        {
            ...
            float sqrDetectionRange = visionRange * visionRange;
            ...
            foreach (Actor otherActor in m_ActorsManager.Actors)
            {
                if (otherActor.Affiliation != actor.Affiliation)
                {
                    ...
                    if (sqrDistance < sqrDetectionRange && sqrDistance < closestSqrDistance)
                    {
                        ...
                        Vector3 directionToTarget = (otherActor.transform.position - DetectionSourcePoint.position).normalized;
                        float angle = Vector3.Angle(DetectionSourcePoint.forward, directionToTarget);
                        ...
                        RaycastHit[] hits = Physics.RaycastAll(DetectionSourcePoint.position,
                            (otherActor.AimPoint.position - DetectionSourcePoint.position).normalized, visionRange,
