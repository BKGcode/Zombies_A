...
{
    public class Feeder : ConsumableStructure
    {
        ...
        {
            if (newState != ChickenState.Eating)
            {
                StopUsing();
        ...
        public Vector3 GetFeedingPosition()
        {
            if (feedingSpot != null)
            {
                return feedingSpot.position;
            }

            return transform.position;
        }

        public bool IsInFeedingRange(Vector3 position)
        {
            float range = gameBalance != null ? gameBalance.feedingRange : 2f;
            return Vector3.Distance(GetFeedingPosition(), position) <= range;
        }
        ...
        private void OnDrawGizmosSelected()
        {
            if (gameBalance != null)
            {
                ...
                Gizmos.DrawWireSphere(GetFeedingPosition(), gameBalance.feedingRange);
            }
        }
        ...
        {
            return uiTexts != null ? uiTexts.feederTitle : "Comedero";
        }

        protected override int GetRefillCost()
        {
            return gameBalance != null ? gameBalance.feederRefillCost : 10;
        }

        protected override int GetRepairCost()
        {
            return gameBalance != null ? gameBalance.feederRepairCost : 20;
        }
        ...
        public void Upgrade()
        {
            if (currentConfig != null && currentConfig.nextLevel != null)
            {
                ...
                UpdateVisual();
                Debug.Log($"[Feeder] Upgraded to Level {currentConfig.level}: {currentConfig.levelName}");
            }
        }
